...
function InvoiceNavigation({ document, onBack, onToCreateOn, counterparts, organizations, contracts, warehouses, operationTypes, onUpdate }){
    ...
    const handleSave = async () => {
        try {
            ...
            const response = await axios.patch(`/api/documents/invoices/${document.id}/update`, dataToSave);
            alert("Рахунок збережено");
            if (onUpdate) onUpdate(response.data);
            console.log("Saved invoice:", response.data);
        } catch (error) {
            console.error("Error saving invoice:", error);
            alert("Помилка при збереженні рахунку");
        }
    };

    const handlePaid = async (closeAfter = false) => {
        try {
            const response = await axios.patch(`/api/documents/invoices/${document.id}/update-status`, {
                status: 'paid'
            });
            ...
            alert("Помилка при оплаті рахунку");
        }
    };
    ...
    return (
        <div className="detail-navigation p-3 bg-white border-bottom shadow-sm">
            <div className="d-flex flex-wrap gap-2 mb-4">
                ...
                <Button variant="success" size="sm" className="btn-icon" onClick={() => handlePaid(false)}>
                    <i className="bi bi-check-circle"></i> Підтвердити оплату
                </Button>
                <Button variant="outline-primary" size="sm" className="btn-icon" onClick={() => handlePaid(true)}>
                    Підтвердити оплату та закрити
                </Button>
                <Dropdown as={ButtonGroup} size="sm">
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item onClick={() => onToCreateOn(document)}>Видаткова накладна</Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
            </div>
